...
async function loadQuestions() {
  try {
    const module = await import('./industry_차체문제.js');
    ...
    roundTitleText = '자동차 차체수리 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry07010${round}`];
            roundTitleText = `자동차 차체수리 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">자동차 차체수리 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>자동차 차체수리 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
