...
async function loadQuestions() {
  try {
    ...
    if (allQuestions.length < 60) {
        ...
        document.getElementById("roundTitle").textContent = `전기전자 통합 랜덤 (${questions.length}문제)`;
    } else {
        // 3. 전체 문제 중 60문제만 랜덤으로 추출
        questions = shuffleArray(allQuestions).slice(0, 60);
        document.getElementById("roundTitle").textContent = `전기전자 통합 랜덤 60문제`;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">전기전자 통합 랜덤 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>전기전자 통합 랜덤 총점: ${score}/${questions.length}</h2>`;
}
