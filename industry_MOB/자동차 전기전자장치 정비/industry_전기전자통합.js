...
async function loadQuestions() {
  try {
    ...
    if (allQuestions.length < 60) {
        ...
        document.getElementById("roundTitle").textContent = `자동차 전기·전자장치 정비 전체랜덤 (${questions.length}문제)`;
    } else {
        // 3. 전체 문제 중 60문제만 랜덤으로 추출
        questions = shuffleArray(allQuestions).slice(0, 60);
        document.getElementById("roundTitle").textContent = `자동차 전기·전자장치 정비 전체랜덤 60문제`;
    }
